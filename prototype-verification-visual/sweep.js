...
(async()=>{const b=await chromium.launch({executablePath:EXE});let fail=0;const ok=m=>console.log('  ok   '+m),bad=m=>{console.log('  FAIL '+m);fail++};
 ...
 const T=[
  ...
  ['compact 390: no overflow in any version',async()=>{await p.click('[data-act="close-confirm"]');await p.setViewportSize({width:390,height:844});let okk=true;for(const v of ['a','b','c']){await p.evaluate(s=>location.hash=s,v);await p.waitForTimeout(250);const r=await p.evaluate(()=>document.documentElement.scrollWidth-document.documentElement.clientWidth);if(r>1){okk=false;console.log('       overflow',v,r)}}await p.evaluate(()=>location.hash='c');await p.screenshot({path:'v-compact-c.png'});await p.setViewportSize({width:1440,height:1000});return okk}],
 ];
