  ...
  function renderWhy(){
    ...
    $("#screen-why").innerHTML =
      ...
      '<h2 class="hb-title-lg">Fifteen ways for B to preview a section\'s data</h2>' +
      '<p class="hb-body-md">All fifteen are read only and all fifteen hand editing to the same Drawer — what differs is how much of the data is on the page, and how much work it is to read. Five were drawn from the page itself; five follow the reference layouts (an insurance card, a smart-home list, dashboard metrics, a payment sheet, a cart summary); and five are the List Item molecule used five ways, since that component already carries a leading slot, a title, a supporting line, an actions row and a trailing slot. Version B carries a switcher so the same order can be seen in each.</p>' +
      ...
      '<p class="hb-body-md"><b>B ships with Activity.</b> It carries no status pills — the row says what happened in words, the readiness banner answers whether the order can go, and a pill would state either a second time. A replaced document shows both files, the version it replaced beside the one now on file. And the order breakdown in the rail is open rather than folded away.</p>' +
      '<p class="hb-body-md"><b>What the others were for:</b> <b>Facts</b> on the storefront, because the labels make a wrong value visible without opening anything, with <b>Summary</b> as the compact-width treatment where the whole page should fit above the fold. <b>Tiles</b> is the strongest of the reference five and the best candidate for phones if the rows ever become tappable. <b>Receipt</b> is the one to try on this page specifically: it speaks with the same voice as the order summary beside it. <b>Credential</b> suits documents more than it suits a branch, <b>Options</b> earns its keep only once an account has a second address, and <b>Highlights</b> is the fastest read but decides for the buyer what matters. <b>As used</b> remains the one to test with buyers, and <b>Record</b> belongs in the dashboard. Of the five built on List Item, <b>List</b> is the one to ship if the answer has to be a component the system already owns, <b>Actions</b> is the most useful to a buyer who came to change one small thing, and <b>Grouped</b> is the only one that still reads well when a section grows past six values.</p>' +
