...
with sync_playwright() as p:
    ...
    ok("A2 verification shown as a value, no upload fields", pg.locator('.verif').count()==1 and pg.locator('.hb-upload').count()==0)
    ...
    ok("P1 receipt uploader is the DS File Upload molecule", pg.locator('.hb-upload').count() == 1)
    ok("P1 uploader carries the persistent header", pg.locator('.hb-upload__title').count() == 1 and pg.locator('.hb-upload__hint').count() == 1)
    ok("P1 uploader shows a live counter", "0 of 1 file" in pg.locator('.hb-upload__count').inner_text())
